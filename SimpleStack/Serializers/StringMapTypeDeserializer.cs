...
using SimpleStack.Serializers.Jsv;
	...
	public class StringMapTypeDeserializer
	{
		...
		public StringMapTypeDeserializer(Type type)
		{
			...
			foreach (var propertyInfo in type.GetSerializableProperties())
			{
				...
				ParseStringDelegate propertyParseStringFn = JsvReader.GetParseFn(propertyType);
			...
			if (SerializationConfig.IncludePublicFields)
			{
				foreach (var fieldInfo in type.GetSerializableFields())
				{
					...
					ParseStringDelegate fieldParseStringFn = JsvReader.GetParseFn(fieldType);
